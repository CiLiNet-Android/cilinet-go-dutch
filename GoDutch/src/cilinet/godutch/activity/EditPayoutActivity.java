...
/** 修改记录 **/
public class EditPayoutActivity extends FrameActivity implements
		View.OnClickListener,NumberDialog.OnNumberDialogListener {
	...
	private void initVariable() {
		...
		mSelectedPayoutModel = (PayoutModel) _intent
				.getSerializableExtra("SelectedPayoutModel");
	...
	private void initView() {
		...
		edtTxt_selectAccountBook.setText(mSelectedPayoutModel.accountBookName);

		btn_selectAccountBook = (Button) findViewById(R.id.btn_selectAccountBook);
		btn_selectAccountBook.setOnClickListener(this);

		mSelectedAccountBookModelId = mSelectedPayoutModel.accountBookID;

		
		// 输入金额
		edtTxt_selectAmount = (EditText) findViewById(R.id.edtTxt_selectAmount);
		edtTxt_selectAmount.setText(mSelectedPayoutModel.amount.toString());
		...
		atoComptEdtTxt_categoryName.setText(mSelectedPayoutModel.categoryName);
		
		btn_selectCategory = (Button)findViewById(R.id.btn_selectCategory);
		btn_selectCategory.setOnClickListener(this);
		
		mSelectedCategoryModelId = mSelectedPayoutModel.categoryID;

		
		// 选择日期
		edtTxt_selectPayoutDate = (EditText) findViewById(R.id.edtTxt_selectPayoutDate);
		edtTxt_selectPayoutDate.setText(DateTools.getFormatDateTime(
				mSelectedPayoutModel.payoutDate, "yyyy-MM-dd"));
		...
		edtTxt_PayoutType.setText(mSelectedPayoutModel.payoutType);
		...
		String _personNames = new PersonBusiness(this)
				.queryPersonNamesByPersonIds(mSelectedPayoutModel.payoutUserID);
		edtTxt_payoutUser.setText(_personNames);
		
		btn_selectUser = (Button)findViewById(R.id.btn_selectUser);
		btn_selectUser.setOnClickListener(this);
		
		mSelectedPayoutUserIds = mSelectedPayoutModel.payoutUserID;
		

		// 备注
		edtTxt_comment = (EditText) findViewById(R.id.edtTxt_comment);
		edtTxt_comment.setText(mSelectedPayoutModel.comment);
	...
	private void addPayout() {
		if (isFormDataAvailable()) {
			...
			_payoutModel.payoutID = mSelectedPayoutModel.payoutID;
			_payoutModel.accountBookID = mSelectedAccountBookModelId;
			_payoutModel.amount = (new BigDecimal(edtTxt_selectAmount.getText()
					.toString().trim()));
			_payoutModel.categoryID = mSelectedCategoryModelId;
			_payoutModel.payoutDate = DateTools.getDate(edtTxt_selectPayoutDate
					.getText().toString().trim(), "yyyy-MM-dd");
			_payoutModel.payoutType = edtTxt_PayoutType.getText().toString()
					.trim();
			_payoutModel.payoutUserID = mSelectedPayoutUserIds;
			_payoutModel.comment = edtTxt_comment.getText().toString().trim();

			boolean _isSuccessed = mPayoutBusiness.updatePayout(_payoutModel);
			if (_isSuccessed) {
	...
	private boolean isFormDataAvailable() {
		// 金额
		if (!RegexTools
				.isMoney(edtTxt_selectAmount.getText().toString().trim())) {
		...
		String[] _payoutTypes = getResources().getStringArray(
				R.array.PayoutType);
		if (null == mSelectedPayoutUserIds || "".equals(mSelectedPayoutUserIds)) {
			...
			if (_selectedPayoutType.equals(_payoutTypes[0])
					|| _selectedPayoutType.equals(_payoutTypes[1])) {
	...
	private void showSelectPayoutUserDialog() {
		...
		_builder.setNegativeButton(R.string.ButtonTextOk,
					...
					public void onClick(DialogInterface dialog, int which) {
						if (mSelectedPayoutUserModels.size() > 0) {
							...
							for (PersonModel _personModel : mSelectedPayoutUserModels) {
								_selectedPersonNames.append(
										_personModel.personName).append(",");

								_selectedPersonIds
										.append(_personModel.personId).append(
												",");
							}
							_selectedPersonNames
									.deleteCharAt(_selectedPersonNames
											.lastIndexOf(","));
							_selectedPersonIds.deleteCharAt(_selectedPersonIds
									.lastIndexOf(","));

							edtTxt_payoutUser.setText(_selectedPersonNames
									.toString());
							mSelectedPayoutUserIds = _selectedPersonIds
									.toString();
		...
		View _view = getLayoutInflater()
				.inflate(R.layout.activity_person, null);
		ListView listV_personList = (ListView) _view
				.findViewById(R.id.listV_personList);
		listV_personList.setAdapter(new PersonAdapter(this));
		listV_personList.setBackgroundColor(getResources().getColor(
				R.color.blue));
		...
		listV_personList
				.setOnItemClickListener(new OnPayoutUserItemClickListener(
						_alertDialog, _payoutType));
		...
		if (null == mSelectedPayoutUserItemViews) {
			mSelectedPayoutUserItemViews = new ArrayList<View>();// 包含了选中的Person
																	// View
		}
	}
	...
	private class OnPayoutUserItemClickListener implements OnItemClickListener {
		...
		public OnPayoutUserItemClickListener(AlertDialog alertDialog,
				String payoutType) {
		...
		public void onItemClick(AdapterView<?> adapterView, View view,
			...
			PersonModel _personModel = (PersonModel) adapterView.getAdapter()
					.getItem(position);

			String[] _payoutTypes = getResources().getStringArray(
					R.array.PayoutType);
			if (null == mPayoutType || "".equals(mPayoutType)
					|| mPayoutType.equals(_payoutTypes[2])) {
	...
	private void showSelectPayoutTypeDialog() {
		...
		View _view = getLayoutInflater().inflate(
				R.layout.payout_type_select_list, null);
		ListView listV_selectPayoutType = (ListView) _view
				.findViewById(R.id.listV_selectPayoutType);
		...
		listV_selectPayoutType
				.setOnItemClickListener(new OnPayoutTypeItemClickListener(
						_alertDialog));
	}
	...
	private class OnPayoutTypeItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> adapterView, View arg1,
				int position, long arg3) {
			String _selectedPayoutType = (String) adapterView.getAdapter()
					.getItem(position);
	...
	private void showSelectPayoutDateDialog() {
		...
		new DatePickerDialog(this, new DatePickerDialog.OnDateSetListener() {
			...
			public void onDateSet(DatePicker view, int year, int monthOfYear,
				...
				String _selectedPayoutDate = DateTools.getFormatDateTime(
						_calendar.getTime(), "yyyy-MM-dd");
	...
	private class OnAccountBookItemClickListener implements
		...
		public void onItemClick(AdapterView<?> adapterView, View arg1,
				int position, long arg3) {
			AccountBookModel _selectedAccountBookModel = (AccountBookModel) adapterView
	...
	private void showSelectCategoryDialog() {
		...
		View _view = getLayoutInflater().inflate(R.layout.category_select_list,
				null);
		ExpandableListView _expandableListView = (ExpandableListView) _view
				.findViewById(R.id.expListV_selectCategory);
		...
		_expandableListView
				.setOnGroupClickListener(new OnCategoryGroupItemClickListener(
						_alertDialog, _categoryAdapter));
		_expandableListView
				.setOnChildClickListener(new OnCategoryChildItemClickListener(
						_alertDialog, _categoryAdapter));
	}
	...
	private class OnCategoryGroupItemClickListener implements
		...
		public OnCategoryGroupItemClickListener(AlertDialog alertDialog,
		...
		public boolean onGroupClick(ExpandableListView parent, View v,
			...
			if (_count == 0) {// 如果没有子类，则可以选择根类
				CategoryModel _categoryModel = (CategoryModel) mCategoryAdapter
	...
	private class OnCategoryChildItemClickListener implements
		...
		public OnCategoryChildItemClickListener(AlertDialog alertDialog,
				CategoryAdapter categoryAdapter) {
			mAlertDialog = alertDialog;
			mCategoryAdapter = categoryAdapter;
		}
		...
		public boolean onChildClick(ExpandableListView parent, View v,
			...
			CategoryModel _selectCategoryModel = (CategoryModel) mCategoryAdapter
